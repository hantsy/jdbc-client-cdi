...
public class JdbcClientSmokeTest {
    ...
    public static void createClient() {
        ...
        config.setPositionalPlaceholderSymbol("?");
        config.setMetricsEnabled(false);

        jdbcClient = new JdbcClient(dataSource, new ConverterRegistry(), config);
    }
    ...
    public void singleValue() {
        Optional<Long> count = jdbcClient.sql("SELECT COUNT(*) FROM engineers").singleValue(Long.class);
    ...
    public void maxRows() {
        List<DevSummary> limited = jdbcClient.sql("SELECT id, dev_name FROM engineers ORDER BY id")
                .maxRows(1).query(DevSummary.class).list();
